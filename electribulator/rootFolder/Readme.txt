...
et les envoyer sur les 5 cannaux de l'electribe afin de polyphonisé tout cela. 
- Permet également de rajouter un systeme complexe d'eneveloppe (VCAs), ainsi que plusieur modulateur (VCOs)assignable a tout les
...
- Arpegiator 
- Il n'est plus nécessaire de toucher a l'electribe tout peut se faire maintenant de votre clavier midi et de votre pc a l'aide de quelques cliques.
...
- Possibilité de sauver/charger les sons et les modulateurs.
...
FR/
- Configuré les synth part de l'electribe chacune sur un canal midi (de 1 a 5).
- Branchez le cable midi ;) etc.
- En théorie maintenant, vous ne devrez plus toucher a l'electribe et vous mettre uniquement sur le pc, et le clavier midi (ou sequenceur?).
- En pratique vous pouvez jouer si vous avez configure l'entrée midi de l'electribe sur le programme afin de recevoir les notes du keyboard et les changement de valeur venant de l'electribe (optionnel)
...
Il vous demande sure quelle entrée midi est branché votre clavier midi (ou autres) configuré sur le canal 1.
Il vous demande ensuite sur quelle sortie midi est mise votre electribe. (ou autres synthé qui serait pareil?).
Il vous demande ensuite sur quelle entrée midi est mise votre electribe. (pour recevoir les event de l'electribe, chg de valeur, notes on-off). (pas obligatoire)
...
FR/
Vous avez maintenant la possiblité de jouer sur votre clavier midi et de normalement constaté que l'electribe 
reagis a au moins une note.
Si ca fonctionne essayez avec plusieur notes en meme temps. Elle devrait normalement active toutes les voix (si 5 notes appuyées)
...
Pour se faire, choisiser un des 4 element modulateur disponible (VCA, VCOs).
	...
	Enveloppe:
		...
		Ouvre ensuite le collapsible panel CCDest pour avoir la config du canal (1 par defaut) et le CC associé
...
- La fréquence theorique d'envoi des commande midi pour les modulations et des +-100 hz actuelement. (Modifie la "bande passante midi")
